...
exports.handleIncomingMessage = exports.SERVICES = exports.DIAS_SEMANA = exports.BotState = exports.conversations = void 0;
const api_client_1 = require("./api-client"); // Importa apenas o objeto da API
...
var BotState;
(function (BotState) {
    BotState[BotState["START"] = 0] = "START";
    BotState[BotState["AWAITING_REGISTRATION_NAME"] = 1] = "AWAITING_REGISTRATION_NAME";
    BotState[BotState["MAIN_MENU"] = 2] = "MAIN_MENU";
    BotState[BotState["EXISTING_APPOINTMENT_MENU"] = 3] = "EXISTING_APPOINTMENT_MENU";
    BotState[BotState["AWAITING_SERVICE_SELECTION"] = 4] = "AWAITING_SERVICE_SELECTION";
    BotState[BotState["AWAITING_DAY_SELECTION"] = 5] = "AWAITING_DAY_SELECTION";
    BotState[BotState["AWAITING_TIME_SELECTION"] = 6] = "AWAITING_TIME_SELECTION";
    BotState[BotState["CONFIRMATION"] = 7] = "CONFIRMATION";
})(BotState = exports.BotState || (exports.BotState = {}));
// Constantes
exports.DIAS_SEMANA = ["Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"];
exports.SERVICES = [
 ...
 */
async function handleStart(conv, input) {
    ...
    conv.activeAppointment = await api_client_1.api.getActiveAppointment(conv.clienteId);
 ...
 */
async function showMainMenu(conv) {
...
async function handleMainMenu(conv, input) {
...
0) Cancelar`;
        ...
        const appointments = await api_client_1.api.getFutureAppointments(conv.clienteId); // Assumimos que a rota existe
 ...
 */
async function handleRegistration(conv, input) {
    const nome = input.trim();
    if (nome.length < 2) {
        return "Por favor, digite um nome válido para o cadastro.";
    }
    const result = await api_client_1.api.createCliente(nome, conv.telefone);
 ...
 */
async function handleExistingAppointmentMenu(conv, input) {
    const selection = parseInt(input);
    if (selection === 1) {
        ...
        await api_client_1.api.cancelAgendamento(conv.activeAppointment.id);
 ...
 */
async function handleServiceSelection(conv, input) {
    const selection = parseInt(input);
    const selectedService = exports.SERVICES.find((s) => s.id === selection);
    if (selection === 0) {
        return await showMainMenu(conv);
    }
    if (!selectedService) {
    ...
    // --- Busca Dias Ativos ---
    const activeDates = await api_client_1.api.getAvailableDates();
    conv.availableDates = activeDates.map((date) => date.split("T")[0]).filter((date) => !!date).slice(0, 8);
    ...
    conv.availableDates.forEach((dateStr, index) => {
        ...
        const dayOfWeek = exports.DIAS_SEMANA[dateObj.getUTCDay()];
 ...
 */
async function handleDaySelection(conv, input) {
    const selection = parseInt(input);
    if (selection === 0) {
        return await showMainMenu(conv);
    }
    ...
    if (!selectedDate) {
        return "Data selecionada inválida. Por favor, tente novamente.";
    }
    conv.selectedDate = selectedDate;
    // CHAMA A ROTA DA API: getAvailableSlots
    const slots = await api_client_1.api.getAvailableSlots(selectedDate);
 ...
 */
async function handleTimeSelection(conv, input) {
    const selection = parseInt(input);
    if (selection === 0) {
        return await showMainMenu(conv);
    }
    ...
    const selectedTime = conv.availableTimes[selection - 1];
    if (!selectedTime) {
        return "Horário selecionado inválido. Por favor, tente novamente.";
    }
    conv.selectedTime = selectedTime;
 ...
 */
async function handleConfirmation(conv, input) {
    const selection = parseInt(input);
    if (selection === 1) {
        ...
        const newAppointmentData = {
            profissionalId: api_client_1.api.getProfissionalId(),
            clienteId: conv.clienteId,
            dataHora: new Date(fullDateTime).toISOString(),
            servico: conv.selectedService,
        };
        const result = await api_client_1.api.createAgendamento(newAppointmentData);
 ...
 */
async function handleIncomingMessage(telefone, message) {
    try {
        let conv;
        const input = message.trim();
        const normalizedInput = input.toLowerCase();
        // 1. Inicializa ou recupera o estado da conversa
        if (!exports.conversations.has(telefone)) {
            const clienteData = await api_client_1.api.getClienteByTelefone(telefone);
            conv = {
            ...
            exports.conversations.set(telefone, conv);
        }
        else {
            conv = exports.conversations.get(telefone);
        }
